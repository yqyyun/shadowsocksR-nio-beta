 ...
 */
public class SalSa20 extends LazyLibsodium {

    public SalSa20(Method cipherMethod, byte[] key, byte[] nonce) {
        super(cipherMethod, key, nonce);
    }

    @Override
    protected void doFinal(byte[] buf, byte[] data, byte[] key, byte[] nonce, long ic) {
        sodiumJava.crypto_stream_salsa20(buf, buf.length, nonce, key);
    }
