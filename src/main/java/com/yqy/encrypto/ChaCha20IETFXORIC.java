 ...
 */
public class ChaCha20IETFXORIC extends LazyLibsodium{
    public ChaCha20IETFXORIC(Method cipherMethod, byte[] key, byte[] nonce) {
        super(cipherMethod, key, nonce);
    }

    @Override
    protected void doFinal(byte[] buf, byte[] data, byte[] key, byte[] nonce, long ic) {
        sodiumJava.crypto_stream_chacha20_ietf_xor_ic(buf, data, data.length, nonce, ic, key);
    }
