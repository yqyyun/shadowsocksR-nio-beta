 ...
 */
public class ChaCha20 extends LazyLibsodium {

    public ChaCha20(Method cipherMethod, byte[] key, byte[] nonce) {
        super(cipherMethod, key, nonce);
    }

    @Override
    protected void doFinal(byte[] buf, byte[] data, byte[] key, byte[] nonce, long ic) {
        sodiumJava.crypto_stream_chacha20(buf, buf.length, nonce, key);
    }
